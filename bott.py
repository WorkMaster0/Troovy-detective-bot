...
# ===== НАЛАШТУВАННЯ =====
TOKEN = os.environ.get("TOKEN")   # 🔑 Токен беремо з Environment Variables Render
...
def init_db():
    if not os.path.exists(DB_NAME):
        logging.warning("Файл БД не знайдено! Створюю новий...")
    # Інший код ініціалізації...
    db_path = os.path.abspath(DB_NAME)
    logging.info(f"Шлях до бази даних: {db_path}")
    if os.path.exists(db_path):
        logging.info("Файл бази даних існує.")
    else:
        logging.info("Файл бази даних не знайдено. Спробую створити.")
...
def handle_help(chat_id):
    msg = ("🆘 Допомога по команді бота 🆘\n"
           ...
           "🚨 /alert btc>50000 - Сповіщення про ціну\n"  # Додано закриваючі лапки
           "💼 /portfolio - Портфель всіх підписок")  # Додано закриваючу дужку
    send_message(chat_id, msg)
